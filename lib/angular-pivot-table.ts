...
@Component({
    selector: 'angular-pivot-table',
    templateUrl: '../views/pivot-table.html'
})
export class PivotTable {
    @Input() columns: Array<string>;
    @Input() rowAttributes: Array<string>;
    @Input() colAttributes: Array<string>;
 ...
 private groupBy(data, groupByAttrs, valueAttrs) {
	const columns = data.shift();
	const groupByAttrsIdxs = [];
	const valueAttrsIdxs = [];
	groupByAttrs.forEach(groupByAttr => {
		groupByAttrsIdxs.push(columns.indexOf(groupByAttr));
	});
	valueAttrs.forEach(valueAttr => {
		valueAttrsIdxs.push(columns.indexOf(valueAttr));
	});
	
	const result = [...data.reduce((r, o) => {
	  const key = groupByAttrsIdxs.reduce((clumative, newValue) => {
		return clumative + o[newValue] + '-';
	  }, '');
	  
	  let item = r.get(key);
	  if(!item) {
		let defaultItem = {};
		groupByAttrs.forEach((groupByAttr, idx) => 
			defaultItem[groupByAttr] = o[groupByAttrsIdxs[idx]]
		);
		valueAttrs.forEach((valueAttr, idx) => {
			defaultItem[valueAttr] = o[valueAttrsIdxs[idx]];
		});

		item = defaultItem;
	  }else {			  
		  valueAttrs.forEach((valueAttr, idx) => {
			item[valueAttr] += o[valueAttrsIdxs[idx]];
		  });
	  }
...
private toTree(data, groupByAttrs) {

	data.reduce((clumativeObj, d) => {
		let f = function(data, currIdx, assignedObj) {
			if(data.children) {
				if(data.children == 0) {
